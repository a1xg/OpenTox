...
import re
import time
#df = pd.read_csv('D:\\python_projects\\food_scanner\\db\\food_ingredients_datafiniti_db.csv', sep=';', encoding='utf-8')
# the standart list of food additions category from wikipedia(national)
start_keywords = ['ingredients']
start_keywords_mask = r'(' + '|'.join(start_keywords) + r')' if len(start_keywords) > 1 else r'(' + start_keywords[0] + r')'
additive_category = ['sweetener', 'humectants', 'flour treatment','coloring', 'bulking', 'antifoaming', 'anticaking',
...
additive_category_mask = re.compile(r'(' + '|'.join(additive_category) + r')')
...
# 'ingridients:' всегда начало состава и можно удалить все что ДО
# 'любоеслово:' не всегда означает состав
# 'слово (0-9)' заменить на 'слово'
# 'слово (слово, слово...)' заменить на 'слово','слово'

# Токенизация: 'слово, слово' надо заменить на 'слово','слово' для легкого преобразования в массив


def stringTokenizer(string:str):
    ...
    start_index = re.search(start_keywords_mask, string, flags=re.IGNORECASE)
    if start_index != None:
    ...
    cleared1 = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\xff-\n]', '', cleared0).lower()
    ...
    cleared7 = re.sub(additive_category_mask, '', cleared6)

    return cleared7,finded_e

start = time.time()
for string in new_dataset:
    print('input:\n', string)
    newstring, e = stringTokenizer(string)
    print('output:\n', newstring, f'\nFinded E chemicals {e}')
    print('_'*100)
end = time.time()
