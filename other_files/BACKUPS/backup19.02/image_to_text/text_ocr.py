...
class TextOCR:
    ...
    def __imagePreprocessing(self):
        ...
        new_max_size = 4000 # set new maximal dimension of image, pix
        ...
        rectKernel = cv2.getStructuringElement(cv2.MORPH_RECT, (20, 40)) # def 20, 25
        sqKernel = cv2.getStructuringElement(cv2.MORPH_RECT, (90, 120)) # def 20, 25
        blackhat = cv2.morphologyEx(imgBlur, cv2.MORPH_BLACKHAT, rectKernel)  # перспективно для увеличения четкости
        ...
        # Тесты повышения резкости
        test_gaussian = cv2.GaussianBlur(grayscale, (47, 47), 10) # норм от (17,17) до (97,97) 10
        test_sharpened = cv2.addWeighted(grayscale, 1.8, test_gaussian, -1.5, 0) # 1.8/-1.5 неплохо улучшают четкость
        test_thresh = cv2.threshold(test_sharpened, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
        # Удаление шумов следует доработать
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
        closing = cv2.erode(test_thresh, kernel, iterations=1)
        opening = cv2.dilate(closing,kernel,iterations = 1)

        #closing = cv2.morphologyEx(test_thresh, cv2.MORPH_OPEN, kernel)


        test_binary_img_for_tesseract = cv2.bitwise_not(opening)

        imshow_size = (int(new_dim[0] / 4), int(new_dim[1] / 4)) # ресайз для тесто
        cv2.imshow('test_sharpened', cv2.resize(test_sharpened, imshow_size, interpolation = cv2.INTER_AREA))
        cv2.imshow("morpho", cv2.resize(opening, imshow_size, interpolation = cv2.INTER_AREA))
        cv2.waitKey(1500)

        outpit_images = (scaled_img, test_binary_img_for_tesseract, paragraph_mask)
