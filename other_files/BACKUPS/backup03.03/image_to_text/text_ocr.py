...
class TextOCR:
    def __init__(self, img:np.ndarray):
        ...
        self.text = [] # список словарей с распознанным текстом в формате {'lang':'text'}
    ...
    def __lineMeter(self, image):
        ...
        for slice in slices:
            ...
            crop = image[0:height, int(newX):int(newX+newW)]
            crop = self.__getTextMask(crop, font_size=0, num_lines=0)
    ...
    def __getBinaryImages(self, image, boxes, font_size, new_font_size:int):
    ...
    def __imageCropper(self, image, crop_coef:float):
        """Метод обрезает картинку пропорционально кроп коэффициенту относительно центра изображения.
    ...
    def __textRecognizer(self, conf:str, image):
    ...
    def getText(self, text_lang:str, lang_detect:bool, crop:bool, set_font:int):
        ...
        if crop == True:
            boxes = self.__findBoxes(mask_img=mask_img, quantity=2)
            binary_images = self.__getBinaryImages(self.img, boxes, font_size, set_font)
        else:
            binary_images = self.__getBinaryImages(self.img, 0, font_size, set_font)
        ...
        for image in binary_images:
            if lang_detect == True:
                ...
                sample_image = self.__imageCropper(image, 0.6)
                multilang_recog_text = self.__textRecognizer(default_config, sample_image)
                # Detect language
                recognized_lang = self.__langDetector(multilang_recog_text)
                custom_config = ('-l ' + recognized_lang + ' --oem 1 --psm 6')
                text = self.__textRecognizer(custom_config, image)
                self.text.append({recognized_lang:text})
            else:
                config = ('-l ' + text_lang + ' --oem 1 --psm 6')
                text = self.__textRecognizer(config, image)
