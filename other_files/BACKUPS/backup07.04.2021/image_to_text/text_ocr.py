...
#  с интерактивными "селект-боксами", которые можно удалять при ненужности, а оставшиеся пускать в распознавание
...
class TextOCR:
    def __init__(self, img:np.ndarray):
        self.img = self._imagePreprocessing(img)
    ...
    def _measureStrings(self, image):
        '''
    ...
    def _getBinaryImages(self, image, boxes, font_size, new_font_size:int):
    ...
    def _findBoxes(self, mask_img, quantity:int):
        ...
        boxes = self._findLargestBoxes(boxes, quantity=quantity) if len(boxes) > quantity else boxes
        return boxes

    def _findLargestBoxes(self, boxes, quantity:int):
    ...
    def getText(self, text_lang:str, lang_detect:bool, crop:bool, set_font:int):
        ...
        :self.lang: the language of the text in the image
         ...
         """
        font_size, num_lines = self._measureStrings(self.img)
        mask_img = self._getTextMask(self.img, font_size, num_lines)
        if crop == True:
            boxes = self._findBoxes(mask_img=mask_img, quantity=2)
            binary_images = self._getBinaryImages(self.img, boxes, font_size, set_font)
        else:
            binary_images = self._getBinaryImages(self.img, 0, font_size, set_font)
