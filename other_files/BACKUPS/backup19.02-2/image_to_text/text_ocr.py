...
class TextOCR:
    ...
    # TODO возмжно стоит на входе ресайзить изображение до 2000пикс делать маску,
    #  а кропнутый текст ресайзить до 1000 пикс и делать препроцессинг для тессеракт
    ...
    def __imagePreprocessing(self):
        ...
        new_max_size = 1024 # set new maximal dimension of image, pix
        ...
        scaled_img = cv2.resize(self.img, new_dim, interpolation = cv2.INTER_CUBIC)
        # morphology methods

        grayscale = cv2.cvtColor(scaled_img, cv2.COLOR_BGR2GRAY)
        imgBlur = cv2.GaussianBlur(grayscale, (3, 3), 0)
        rectKernel = cv2.getStructuringElement(cv2.MORPH_RECT, (20, 25)) # def 20, 25
        sqKernel = cv2.getStructuringElement(cv2.MORPH_RECT, (20, 25)) # def 20, 25
        ...
        # Удачные эксперименты повышения резкости, все настроено под разрешение картинки с текстом 1024
        ...
        #cv2.imshow("blur thresh", test_thresh)
        #cv2.waitKey(1000)
