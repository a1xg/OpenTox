...
for dir in file_dir:
    ...
    text_cleaner = TextPostprocessing(input_dicts=ocr.text)
    print(f'Cleared text:\n {text_cleaner.output_dicts}')
    # перебираем словари с текстом и отправляем в модуль базы данных
    for dict in text_cleaner.output_dicts:
        test = Database_API()
        test.unpackKeywords(data=dict)
        test.getData()
        print(f'Finded in database {test.results}')
